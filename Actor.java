...
public class Actor {
	...
	public Actor(Theatre theatre) {    // Конструктор Актера
	...
	public Actor(String name, String surname, int age, Theatre theatre) { // Конструктор Актреа с параметрами
		this.nameAct = name;
	...
	public String getSurnameAct() {  // Возвращение фамилии Актера
		return surnameAct;
	}

	public int getAgeAct() {         // Возвращение возраста Актреа
		return ageAct;
	}

	public Theatre getJobPlace() {   // Возвращение места работы Актера
		return jobPlace;
	}

	public Performance getPerformanceAct(int num) {  // Возвращение постановки с участ. Актера
		return performanceAct[num];
	}

	public int getRoles() { // Возвращение кол-ва ролей
		return roles;
	}

	public void toPrint() {  // Вывод информации об Актере
	...
	public void addPerformanceAct(Performance performance) { // Добавление Постановки в список ролей
		performanceAct[getRoles()] = performance;
		performanceAct[getRoles()].addActor();
		addRoles();
	}

	public void delPerformanceAct(Performance performance) { // Удаление Постановки из списка ролей
	boolean flag = false;
	int numPerf = 0;
	for (int i = 0; i < getRoles(); i++)
	...
	private String toWriteSurnameAct() {                // Ввод фамилии Актера
	...
	private int toWriteAgeAct() {                       // Ввод возраста Актера
	...
	private void addRoles() { // Добавление роли
		roles++;
	}

	private void delRoles() { // Удаление роли
		roles--;
	}
}